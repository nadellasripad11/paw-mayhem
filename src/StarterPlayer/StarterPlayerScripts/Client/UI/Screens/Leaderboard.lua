...
-- Weekly/Friends tabs are shown for layout parity with the reference but are
-- clearly marked "coming soon" rather than faking data.)
...
local Leaderboard = {}
local boardList, statsHolder, tabButtons, comingSoonLabel
...
local function rebuildBoard()
	...
	boardList.Visible = (activeTab == "Global")
	comingSoonLabel.Visible = (activeTab ~= "Global")
...
function Leaderboard.Build(parent)
	...
	tabButtons = {}
	for _, name in ipairs({ "Global", "Weekly", "Friends" }) do
	...
	comingSoonLabel = UIUtil.label({ Parent = listWrap, Text = "Coming soon — this needs cross-server tracking.", TextColor3 = Theme.Color.TextMuted, TextSize = 13, TextXAlignment = Enum.TextXAlignment.Center, Size = UDim2.new(1, 0, 0, 40), Position = UDim2.fromOffset(0, 20), Visible = false })
	rebuildBoard()
